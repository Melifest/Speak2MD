...
def generate_markdown(job_id: str, raw_text: str, language: str = "ru") -> str:
    ...
    Задача: принять сырой русский ASR-текст (оговорки, повторы, шумы, сбитая пунктуация) и выдать ОЧИЩЕННУЮ версию — связный литературный текст с минимальной структурой. Смысл не менять, факты не добавлять. Итоговый объём — ≥95% от исходного по символам (с пробелами). Если выходит <90%, разверни сокращённые места ИЗ исходника (перефразируй без новых фактов).

    Выход: только чистый текст (Markdown допустим для заголовков/формул), без служебных метаданных и без JSON.

    Правила:
    - Сохраняй ≥90% содержания; порядок тем в целом — как в исходнике, фразы можно немного переставлять ради связности.
    ...
    - Восстанавливай пунктуацию, регистр, опечатки, согласование; используй «ёлочки», корректные тире.
